...
The CI gate (`.github/workflows/ci.yml`) runs exactly these plus two
simulation-only smoke tests; see "CI and the test stack" below. There is no
linter, formatter, or type checker. Tests use the standard library `unittest`
only -- do not introduce pytest as an application dependency.
...
- `unittest` only, standard library; do not introduce pytest as an app
  dependency. Tests live in `tests/test_<module>.py`, classes named
...
the full `unittest discover` run, and two headless `--simulate` smoke tests.

One nuance: `tests/test_radio_write_readback_probe.py` is pytest-style (20
function tests invisible to `unittest discover`), so CI installs `pytest` as a
**CI-only** test dependency and runs that module separately. Keep
`requirements.txt` app-only; do not add `pytest` to it.
...
- **Do not merge or open a pull request unless explicitly instructed to.** A
  coding agent may push a topic branch, but never merge it into `main` on its
  own initiative.
- **Hardware-only validation must be reported honestly.** If a change has only
  been validated in `--simulate`, say so; never claim a real-radio
  result that was not actually observed. When a behavior is hardware-only and
  was not run, label it as unverified rather than passing.
